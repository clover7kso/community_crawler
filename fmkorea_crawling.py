...
def getData():
    ...
    while True:
        ...
        for i in soup.find_all('tr'):
            ...
            num = url[url.find('document_srl='):].replace(
                "document_srl=", "").strip()
            cursor.execute(sql, ("에펨코리아", num, url, title, replyNum, viewNum, voteNum, timeValue.strftime("%Y-%m-%d %H:%M:%S"),
                                 url, title, replyNum, viewNum, voteNum, timeValue.strftime("%Y-%m-%d %H:%M:%S")))
            print("에펨코리아-", num, " URL : ", url, "제목 : ", title, " 댓글수 : ", replyNum, " 시간 : ", timeValue.strftime("%Y-%m-%d %H:%M:%S"),
