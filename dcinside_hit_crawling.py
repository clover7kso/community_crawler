...
def getData():
    ...
    while True:
        ...
        for i in soup.find_all('tr'):
            ...
            cursor.execute(sql, ("디씨 HIT", num, url, title, replyNum, viewNum, voteNum, timeValue.strftime("%Y-%m-%d %H:%M:%S"),
                                 url, title, replyNum, viewNum, voteNum, timeValue.strftime("%Y-%m-%d %H:%M:%S")))
            print("디씨 HIT-", num, " URL : ", url, "제목 : ", title, " 댓글수 : ", replyNum, " 시간 : ", timeValue.strftime("%Y-%m-%d %H:%M:%S"),
